...
To reduce Render free-tier cold starts, the Blueprint includes a Render cron
service named `tutr-api-keep-awake`. It runs every 10 minutes and pings the
backend health endpoint using `TUTR_API_HEALTH_URL`, currently set to
`https://tutr-api.onrender.com/api/v1/health` in `render.yaml`.

The repo also includes `.github/workflows/keep-api-awake.yml` as a GitHub
Actions fallback. If you use the Render cron service, you can leave the GitHub
`TUTR_API_HEALTH_URL` variable unset to avoid duplicate pings.
